...
class DiagnosesList extends Component {
    constructor(props) {
        super(props);
    ...
    get() {
        ...
        fetch('http://127.0.0.1:4000/admin/diagnoses', {
            method: 'GET',
            headers: {
    ...
    delete(id) {
        ...
        fetch('http://127.0.0.1:4000/admin/diagnoses/' + id, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
                        ...
                        this.state.items.map((item, idx) => {
                            return (
                                <Row className="table-row" key={idx}>
                                    ...
                                    <Col lg="3" className="actions">
                                        <Link to={`/admin/diagnoses/${item._id}`}><Isvg src={editIcon} /></Link>
...
export default Page(DiagnosesList);